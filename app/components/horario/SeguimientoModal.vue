    ...
    <template #body>
      <div v-if="selectedItem" class="space-y-4">
        <div v-if="(selectedItem as any).carga?.docente || (selectedItem as any).docente_nombre">
          <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Docente</h4>
        ...
        <div class="grid grid-cols-3 gap-4">
          <div>
          <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Curso</h4>
          ...
          <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Sede</h4>
            <p class="text-base text-gray-900 dark:text-gray-100">
              {{ (selectedItem as any).sede || (selectedItem as any).sede_nombre || "N/A" }}
            </p>
          </div>
          <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Grupo</h4>
            <p class="text-base text-gray-900 dark:text-gray-100">
        ...
        <USeparator v-if="isAdmin" class="my-6" />
        ...
        >
          <div class="bg-gradient-to-r from-emerald-500/10 to-teal-500/5 px-4 py-3 border-b border-emerald-100 dark:border-emerald-900/30 flex items-center gap-2">
            <UIcon name="i-lucide-eye" class="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
          ...
          <div class="p-4 space-y-4">
            ...
            >
              <template #historial>
               <div v-if="historial.length > 0 || isLoadingHistorial" class="space-y-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                <div class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
                  <UIcon name="i-lucide-history" class="w-4 h-4" />
                  <h4 class="text-xs font-bold uppercase tracking-wider">Historial de Seguimiento</h4>
                  <UIcon v-if="isLoadingHistorial" name="i-lucide-loader-2" class="w-3 h-3 animate-spin ml-auto" />
                </div>
                    ...
                    >
                      <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                          <UBadge size="xs" variant="subtle" :color="obs.tipo === 'Desvinculación' ? 'error' : 'warning'">
                            {{ obs.estado }}
                          </UBadge>
            ...
            <div v-if="responseSeguimiento" class="mt-4 border border-cyan-500/30 rounded-lg p-3 bg-cyan-500/5">
